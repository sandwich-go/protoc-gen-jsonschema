...
func (c *Converter) convertFile(file *descriptor.FileDescriptorProto) ([]*plugin.CodeGeneratorResponse_File, error) {
	...
	var msgTypes []*descriptor.DescriptorProto
	for _, v := range file.GetMessageType() {
		if src := c.sourceInfo.GetMessage(v); src != nil {
			comments := formatDescription(src)
			c.logger.Debug(comments)
			if strings.Contains(comments, "@schema") {
				msgTypes = append(msgTypes, v)
			}
		}
	}
	var enumTypes []*descriptor.EnumDescriptorProto
	for _, v := range file.GetEnumType() {
		if src := c.sourceInfo.GetEnum(v); src != nil {
			enumTypes = append(enumTypes, v)
		}
	}
	c.logger.Debugf("msg:%d enum:%d", len(msgTypes), len(enumTypes))
	// Warn about multiple messages / enums in files:
	if len(msgTypes) > 1 {
		c.logger.WithField("schemas", len(msgTypes)).WithField("proto_filename", protoFileName).Warn("protoc-gen-jsonschema will create multiple MESSAGE schemas from one proto file")
	}
	if len(enumTypes) > 1 {
		c.logger.WithField("schemas", len(enumTypes)).WithField("proto_filename", protoFileName).Warn("protoc-gen-jsonschema will create multiple ENUM schemas from one proto file")
	}
	...
	if len(msgTypes) == 0 {
		...
		for _, msg := range msgTypes {
			...
			// Convert the message:
			messageJSONSchema, err := c.convertMessageType(pkg, msg, enumTypes)
			if err != nil {
				c.logger.WithError(err).WithField("proto_filename", protoFileName).Error("Failed to convert")
				return nil, err
			}
